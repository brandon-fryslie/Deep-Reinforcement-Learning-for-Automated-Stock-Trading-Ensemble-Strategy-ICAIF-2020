...
def get_turbulence_threshold(df, unique_trade_date, rebalance_window, validation_window, curr_start_idx: int):
    ...
    end_date_index = df.index[df["datadate"] == unique_trade_date[curr_start_idx]].to_list()[-1]
    ...
    print(f"get_turbulence_threshold: {turbulence_threshold}")

    return turbulence_threshold

def run_single_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    strat_name = "A2C"
    ...
    for i in range(rebalance_window + validation_window, range_end_idx, rebalance_window):
        ...
        curr_end_idx = i - rebalance_window

        info_log("============================================")
        info_log(f"Window Idx: '{i}' Date start: '{curr_start_idx}' Date End: '{curr_end_idx}'")
        info_log("============================================")
        ...
        turbulence_threshold = get_turbulence_threshold(df, unique_trade_date, rebalance_window, validation_window, curr_start_idx)

        ############## Environment Setup starts ##############
        ## training env
        info_log(f"Environment Setup Start: {20090000} - {unique_trade_date[curr_start_idx]}")
        train = data_split(df, start=20090000, end=unique_trade_date[curr_start_idx])
        env_train = DummyVecEnv([lambda: StockEnvTrain(train)])

        ## validation env
        validation = data_split(df, start=unique_trade_date[curr_start_idx],
                                end=unique_trade_date[curr_end_idx])
        env_val = DummyVecEnv([lambda: StockEnvValidation(validation,
                                                          turbulence_threshold=turbulence_threshold,
                                                          iteration=i)])
        obs_val = env_val.reset()
        ############## Environment Setup ends ##############
        print(f"======{strat_name} Training========\n")

        model = train_model(strat_name, env_train, model_name=f"{strat_name}_100k_dow_{i}", timesteps=100000)

        print(f"======{strat_name} Validation from: ", unique_trade_date[curr_start_idx], "to ",
              unique_trade_date[curr_end_idx])
        DRL_validation(model=model, test_data=validation, test_env=env_val, test_obs=obs_val)
        sharpe = get_validation_sharpe(i)
        print(f"{strat_name} Sharpe Ratio: ", sharpe)



        ############## Trading starts ##############
        print("\n!!!!!! ############## Trading starts ############## \n")
        print("\n======Trading from: ", unique_trade_date[curr_end_idx], "to ", unique_trade_date[i])

        last_state_ensemble = DRL_prediction(df=df, model=model, name="ensemble",
    ...
    info_log(f"\n !!!!! Single Strategy ({strat_name}) took: {(end - start) / 60} minutes")
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        print("============================================")
        ## initial state is empty
        if curr_end_idx - validation_window == 0:
            # inital state
            initial = True
        else:
            # previous state
        ...
        # Tuning turbulence index based on historical data
        # Turbulence lookback window is one quarter
        end_date_index = df.index[df["datadate"] == unique_trade_date[curr_end_idx - validation_window]].to_list()[-1]
        start_date_index = end_date_index - validation_window*30 + 1

        historical_turbulence = df.iloc[start_date_index:(end_date_index + 1), :]
        #historical_turbulence = df[(df.datadate<unique_trade_date[curr_end_idx - validation_window]) & (df.datadate>=(unique_trade_date[curr_end_idx - validation_window - 63]))]
        ...
        ############## Environment Setup starts ##############
        ## training env
        train = data_split(df, start=20090000, end=unique_trade_date[curr_end_idx - validation_window])
        env_train = DummyVecEnv([lambda: StockEnvTrain(train)])

        ## validation env
        validation = data_split(df, start=unique_trade_date[curr_end_idx - validation_window],
                                end=unique_trade_date[curr_end_idx])
        env_val = DummyVecEnv([lambda: StockEnvValidation(validation,
                                                          turbulence_threshold=turbulence_threshold,
                                                          iteration=i)])
        ...
        print("\n======Model training from: ", 20090000, "to ",
              unique_trade_date[curr_end_idx - validation_window])
        # print("training: ",len(data_split(df, start=20090000, end=test.datadate.unique()[i-rebalance_window]) ))
        # print("==============Model Training===========")
        print("======A2C Training========\n")
        model_a2c = train_A2C(env_train, model_name="A2C_30k_dow_{}".format(i), timesteps=30000)
        print("======A2C Validation from: ", unique_trade_date[curr_end_idx - validation_window], "to ",
              unique_trade_date[curr_end_idx])
        ...
        print("======PPO Validation from: ", unique_trade_date[curr_end_idx - validation_window], "to ",
              unique_trade_date[curr_end_idx])
        ...
        print("======DDPG Validation from: ", unique_trade_date[curr_end_idx - validation_window], "to ",
              unique_trade_date[curr_end_idx])
